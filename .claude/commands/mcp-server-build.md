# /mcp-server-build - Build Braiins OS MCP Tools (Skills-First)
...
Develop new MCP tools for the Braiins OS server using skills-first approach. Single agent loads `mcp-builder` skill and follows the 4-phase workflow to build production-ready tools that enable AI agents to manage ASIC miners. Ideal for adding 1-5 related tools to the existing server.
...
**For Braiins OS Server**:
- Project context already loaded (braiins-os-mcp-server)
- Service: Braiins OS API (gRPC + REST)
- API Docs: https://github.com/braiins/bos-plus-api
- Existing tools: authenticate, list_miners, get_miner_status, update_pool_config, update_firmware

**Agent will ask**:
- What new tool(s) do you want to add?
- If > 5 tools, suggests `/mcp-orchestrate-server` for parallel development
...
3. **Study Braiins OS API Documentation**:
   - Use WebFetch: https://raw.githubusercontent.com/braiins/bos-plus-api/main/README.md
   - gRPC service definitions and endpoints
   - Available miner operations (status, config, firmware)
   - Authentication via miner credentials
   - Data models: MinerStatus, PoolConfig, FirmwareInfo
...
2. **Implement Core Infrastructure**:
   ...
   **Shared Utilities** (Already Implemented):
   ```typescript
   // src/api/grpc/pool.ts - Connection pooling
   export class GrpcConnectionPool {
     async getConnection(minerId: string): Promise<MinerServiceClient>
   }

   // src/api/grpc/retry.ts - Retry logic
   export async function withRetry<T>(fn: () => Promise<T>): Promise<T>

   // src/cache/redis.ts - Caching layer
   export class RedisCache {
     async get(key: string): Promise<string | null>
     async setex(key: string, ttl: number, value: string): Promise<void>
   }

   // src/utils/formatters.ts - Response formatting
   export function formatMinerStatus(
     status: MinerStatus,
